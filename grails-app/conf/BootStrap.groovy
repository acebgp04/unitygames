...
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        String path = !System.properties['os.name'].toLowerCase().contains('windows') ? "/Users/ace/Desktop/unity/players.xlsx" : "C:\\MASTER FILE (1).xls"
        ...
        booksMapList.each { Map playerParams ->
            ...
            new Player(controlNo: "${playerParams.get('controlNo').toString().trim()}",
                       sport: sport,
                       category: category,
                       team: Team.findByName(playerParams.get('team')) ?: new Team(name:playerParams.get('team')).save(flush: true),
                       fullName: playerParams.get('lastName') + ", " + playerParams.get('firstName'),
                       firstName: playerParams.get('firstName'),
                       lastName: playerParams.get('lastName'),
